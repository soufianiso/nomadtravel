...
func (s *Userservice) RegisterUser(ctx context.Context, req *userpb.RegisterUserRequest) (*userpb.RegisterUserResponse, error) {
	...
	if err !=  nil{
		s.log.Error("problem in databsase", "Error", err)
		return nil, err
	}
	if exists{
		s.log.Error("email already exists", "Error", err)
		return nil, err
	}
	...
	if err !=  nil{
		s.log.Error("Failed to Create the user", "Error", err)
		return nil, err
	}
...
func (s *Userservice) LoginUser(ctx context.Context, req *userpb.LoginUserRequest) (*userpb.LoginUserResponse, error){
	...
	if err != nil{
		s.log.Error("failed to verify the email", "Error", err)
		return nil, err
	}
	s.log.Info("found the email and its hashed password","email", email , "hashedpassword", hashedpassword)

	err = validatePassword(hashedpassword, req.GetPassword()) 
	if err != nil{
		s.log.Error("failed to compared the hashed password against plain password", "Error", err)
		return nil, err
	}
	...
	if err != nil{
		s.log.Error("Failed to Create a token","email",email)
		return nil, err
	}
